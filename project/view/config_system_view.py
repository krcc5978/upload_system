...
class ConfigSystemView:
    ...
    def __init__(self, master, config_path):
        ...
        self.dirctory = InitParts(master, 'ディレクトリ', 3, 0, (10, 0), (10, 0), self.config.directory)
        self.login_id = InitParts(master, 'ID', 4, 0, (10, 0), (10, 0), self.config.login_id)
        self.login_pass = InitParts(master, 'パスワード', 5, 0, (10, 0), (10, 0), self.config.login_pass)
        ...
        self.update_button = tk.Button(master, text=u'更新', font=('Helvetica', 8),
                                       command=lambda: update_config(self.config,
                                                                     ...
                                                                     self.dirctory.entry.get(),
                                                                     self.login_id.entry.get(),
                                                                     self.login_pass.entry.get(),
                                                                     ))
        self.update_button.grid(row=6, column=2, padx=(10, 0), sticky=tk.W)
